...
export class AiCommentService {
  ...
  async getAiComment(userId: string): Promise<AiCommentResponseDto> {
    ...
    const overspent = totalSpend > shoppingBudget;

    // Prompt 생성
    const prompt = !overspent
      ...
      말투는 친근하고 뿌듯한 느낌으로 작성해 주세요. 문장 길이는 약 80자로 해주세요.`
      : `이번 달 소비 목표: ${shoppingBudget}원
      이번 달 실제 소비: ${totalSpend}원

      소비를 초과했음을 언급하며 한 문장으로 후기를 작성해 주세요.
      조금 아쉬운 느낌의 말투로 작성해 주세요. 문장 길이는 약 80자로 해주세요.`;
    ...
    const type: "positive" | "negative" = overspent ? "negative" : "positive";
