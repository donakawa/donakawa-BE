...
export class LogRepository {
  ...
  public async getDroppedAutoItemsLast3Months(userId: string, startDate: Date) {
    const userIdBigInt = BigInt(userId);
    return this.prisma.addedItemAuto.findMany({
      where: {
        userId: userIdBigInt,
  ...
  public async getItemStatusCounts(userId: string) {
    ...
    return {
      total: autoWishlisted + manualWishlisted,

      bought: autoBought + manualBought,

      dropped: autoDropped + manualDropped,
    };
  }
  ...
  public async updateGoalStatus(goalId: number, status: GoalStatus) {
  ...
  public async findPastGoals(userId: string) {
    const userIdBigInt = BigInt(userId);
    ...
    return this.prisma.goal.findMany({
      where: {
        userId: userIdBigInt,
        status: {
          in: [GoalStatus.COMPLETED, GoalStatus.STOPPED],
        },
      },
      orderBy: {
        endedAt: "desc",
